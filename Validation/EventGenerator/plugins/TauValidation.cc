...
void TauValidation::bookHistograms(DQMStore::IBooker &i, edm::Run const &, edm::EventSetup const &){
  ///Setting the DQM top directories
    ...
    pi0NDau=dqm.book1dHisto("pi0NDau","Number of daughters for #pi^{0}'s",6,-0.5,5.5);
    etaNDau=dqm.book1dHisto("etaNDau","Number of daughters for #eta's ",6,-0.5,5.5);
    K0NDau =dqm.book1dHisto("K0NDau","Number of daughters for #K^{0}'s",6,-0.5,5.5);
...
void TauValidation::analyze(const edm::Event& iEvent,const edm::EventSetup& iSetup){ 
  ...
  for (reco::GenParticleCollection::const_iterator iter = genParticles->begin(); iter != genParticles->end(); ++iter) {
	...
	if(mother>-1){ // exclude B, D and other non-signal decay modes
	  ...
	  photons(&(*iter),weight);
	  myflag=false;
	  CountDaughters(&(*iter),weight);
	  if(myflag) std::cout << "iEvent.id().event(): " << iEvent.id().event() << " iEvent.id().run(): " << iEvent.id().run() << std::endl;
	  ///////////////////////////////////////////////
	  ...
	  if(TD.AnalyzeTau(&(*iter),jak_id,TauBitMask,false,false)){
		...
		if(abs(part.at(i)->pdgId())!=PdtPdgMini::nu_tau && TD.isTauFinalStateParticle(part.at(i)->pdgId()) && !hasDL){
		  ...
		  TVector3 DL=SV-PV;
		  if(myflag) std::cout << "DL: " << DL.X() << " " << DL.Y() << " " << DL.Z() << std::endl;
...
void TauValidation::CountDaughters(const reco::GenParticle* p, double weight){
  for(unsigned int i = 0; i <p->numberOfDaughters(); i++){
    const reco::GenParticle *dau=static_cast<const reco::GenParticle*>(p->daughter(i));
    if(isLastTauinChain(dau)){
      if(abs(dau->pdgId())==111){
	pi0NDau->Fill(dau->numberOfDaughters(),weight);
        myflag=true;
	std::cout << "pi0: " <<  dau->pdgId() << " status: " << dau->status() << " ndau: " << dau->numberOfDaughters() << std ::endl;
        for(unsigned int i = 0; i <dau->numberOfDaughters(); i++){
          const reco::GenParticle *dau2=static_cast<const reco::GenParticle*>(dau->daughter(i));
	  std::cout << "pi0 dau: " <<  dau2->pdgId() << " status: " << dau2->status() << " ndau: " << dau2->numberOfDaughters() << std ::endl;
        }

      }
      if(abs(dau->pdgId())==221){
	etaNDau->Fill(dau->numberOfDaughters(),weight);
	myflag=true;
	std::cout << "eta: " <<  dau->pdgId() << " status: " << dau->status() << " ndau: " << dau->numberOfDaughters() << std ::endl;
        for(unsigned int i = 0; i <dau->numberOfDaughters(); i++){
          const reco::GenParticle *dau2=static_cast<const reco::GenParticle*>(dau->daughter(i));
	  std::cout << "eta dau: " <<  dau2->pdgId() << " status: " << dau2->status() << " ndau: " << dau2->numberOfDaughters() << std ::endl;
        }
      }
      if(abs(dau->pdgId())==130 || abs(dau->pdgId())==310 || abs(dau->pdgId())==311){
	K0NDau->Fill(dau->numberOfDaughters(),weight);
	myflag=true;
	std::cout << "K0: " <<  dau->pdgId() << " status: " << dau->status() << " ndau: " << dau->numberOfDaughters() << std ::endl;
	for(unsigned int i = 0; i <dau->numberOfDaughters(); i++){
	  const reco::GenParticle *dau2=static_cast<const reco::GenParticle*>(dau->daughter(i));
	  std::cout << "K0 dau: " <<  dau2->pdgId() << " status: " << dau2->status() << " ndau: " << dau2->numberOfDaughters() << std ::endl;
	}
      }
    }
    CountDaughters(dau,weight);
  }
}
