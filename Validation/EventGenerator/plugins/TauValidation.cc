  ...
  for (reco::GenParticleCollection::const_iterator iter = genParticles->begin(); iter != genParticles->end(); ++iter) {
	...
	if(mother>-1){ // exclude B, D and other non-signal decay modes
	  ...
	  if(TD.AnalyzeTau(&(*iter),jak_id,TauBitMask,false,false)){
	    ...
	    int decaychannel = tauDecayChannel(&(*iter),jak_id,TauBitMask,weight);
		...
		if(abs(part.at(i)->pdgId())!=PdtPdgMini::nu_tau && TD.isTauFinalStateParticle(part.at(i)->pdgId()) && !hasDL){
		  ...
		  double c(2.99792458E8),Ltau(DL.Mag()/1000),beta(iter->p()/iter->mass());
		...
		if(TD.isTauFinalStateParticle(part.at(i)->pdgId()) && abs(part.at(i)->pdgId())!=PdtPdgMini::nu_e &&
		  ...
		  if(jak_id==TauDecay::JAK_A1_3PI ||
		    ...
		    if((tcharge==part.at(i)->pdgId()/abs(part.at(i)->pdgId()) && TD.nProng(TauBitMask)==3) || (jak_id==TauDecay::JAK_A1_3PI && TD.nProng(TauBitMask)==1 && abs(part.at(i)->pdgId())==PdtPdgMini::pi_plus) ){
		      LVS13+=LV;
		      LVS23+=LV;
		    }
		    else{
		      LVS12+=LV;
		      if(!haspart1 && ((jak_id==TauDecay::JAK_A1_3PI)  || (jak_id!=TauDecay::JAK_A1_3PI && abs(part.at(i)->pdgId())==PdtPdgMini::K_plus) )){
			LVS13+=LV;
			haspart1=true;
		      }
		      else{
...
int TauValidation::tauDecayChannel(const reco::GenParticle* tau,int jak_id, unsigned int TauBitMask, double weight){
  ...
  // resonances  
  TauDecay_GenParticle TD;
  if(jak_id==TauDecay::JAK_KAON)     channel = K;
  if(TD.hasResonance(TauBitMask,PdtPdgMini::K_star_plus)) channel = Kstar;
  if(TD.hasResonance(TauBitMask,PdtPdgMini::a_1_plus))    channel = a1;
  if(TD.hasResonance(TauBitMask,PdtPdgMini::rho_plus))    channel = rho;
  if(channel!=undetermined && weight!=0.0) TauDecayChannels->Fill(undetermined,weight);
  
  // final state products
  if(jak_id==TauDecay::JAK_PION)                               channel = pi;
  if(jak_id==TauDecay::JAK_RHO_PIPI0)                          channel = pi1pi0;
  if(TD.nPi0(TauBitMask)>1)                                    channel = pinpi0;
  if(TD.nProng(TauBitMask)==3 && jak_id==TauDecay::JAK_A1_3PI) channel = tripi;
  if(jak_id==TauDecay::JAK_3PIPI0)                             channel = tripinpi0;
  if(jak_id==TauDecay::JAK_ELECTRON)                           channel = electron;
  if(jak_id==TauDecay::JAK_MUON)                              channel = muon;
  if(weight!=0.0)                                              TauDecayChannels->Fill(channel,weight);
  return channel;
}

void TauValidation::rtau(const reco::GenParticle* tau,int mother, int decay, double weight){
  if(decay != pi1pi0) return; // polarization only for 1-prong hadronic taus with one neutral pion to make a clean case
  if(tau->pt() < tauEtCut) return; // rtau visible only for boosted taus
  double rTau = 0;
...
void TauValidation::spinEffectsZH(const reco::GenParticle* boson, double weight){
  if(abs(boson->pdgId())==PdtPdgMini::Z0 || abs(boson->pdgId())==PdtPdgMini::Higgs0){
	...
	if(TD.AnalyzeTau(dau,jak_id,TauBitMask,false,false)){
	  std::vector<const reco::GenParticle*> part=TD.Get_TauDecayProducts();
	  if(jak_id==TauDecay::JAK_PION || jak_id==TauDecay::JAK_MUON || jak_id==TauDecay::JAK_ELECTRON){
	    if(jak_id==TauDecay::JAK_PION)     nSinglePionDecays++;
	    if(jak_id==TauDecay::JAK_MUON)     nSingleMuonDecays++;
	    if(jak_id==TauDecay::JAK_ELECTRON) nSingleElectronDecays++;
	    ...
	    if(jak_id==TauDecay::JAK_PION){
	      if(abs(boson->pdgId())==PdtPdgMini::Z0) TauSpinEffectsZ_X->Fill(LVpi.P()/LVtau.E(),weight);
	  ...
	  if(jak_id==TauDecay::JAK_RHO_PIPI0){
	    for(unsigned int i=0; i<part.size();i++){
    ...
    if(nSingleMuonDecays==2){
      if(abs(boson->pdgId())==PdtPdgMini::Z0)     TauSpinEffectsZ_muX->Fill(x1,weight);
      if(abs(boson->pdgId())==PdtPdgMini::Higgs0) TauSpinEffectsH_muX->Fill(x1,weight);
    }
    if(nSingleElectronDecays==2){
      if(abs(boson->pdgId())==PdtPdgMini::Z0)     TauSpinEffectsZ_eX->Fill(x1,weight);
      if(abs(boson->pdgId())==PdtPdgMini::Higgs0) TauSpinEffectsH_eX->Fill(x1,weight);
    }
