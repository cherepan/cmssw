...
void TauValidation::beginJob()
{
  if(dbe){
    ...
    TauSpinEffectsH_Xb   = dbe->book1D("TauSpinEffectsHXb","X of backward emitted #tau^{-}", 25 ,0,1.0);           TauSpinEffectsZ_Xb->setAxisTitle("X_{b}");
     ...
     TauSpinEffectsH_phistarminus = dbe->book1D("TauSpinEffectsH_phistarminus","#phi^{*} (acoplanarity) for Higgs #rightarrow #rho-#rho (y_{1}*y_{2}<0)", 32 ,0,2*TMath::Pi());     TauSpinEffectsH_phistarminus->setAxisTitle("#phi^{*}");
     TauSpinEffectsH_phistarplus = dbe->book1D("TauSpinEffectsH_phistarplus","#phi^{*} (acoplanarity) for Higgs #rightarrow #rho-#rho (y_{1}*y_{2}>0)", 32 ,0,2*TMath::Pi());     TauSpinEffectsH_phistarplus->setAxisTitle("#phi^{*}");
 ...
 void TauValidation::spinEffectsZH(const HepMC::GenParticle* boson, double weight){
   ...
   if(hasrho_minus && hasrho_plus){
     ...
     std::cout << "TauValidation " << pi_rhominus.E() << " " << pi0_rhominus.E() << " " << pi_rhoplus.E() << " " << pi0_rhoplus.E() << " " << std::endl;
     // boost to rhorho cm
     ...
     std::cout << "TauValidation " <<  pi_rhominus.Vect().Dot(n_plus) << " " << pi_rhoplus.Vect().Dot(n_minus) << std::endl; 
     ...
     // fill histograms
     if(abs(boson->pdg_id())==PdtPdgMini::Higgs0 && y1*y2<0) TauSpinEffectsH_phistarminus->Fill(phistar,weight);
     if(abs(boson->pdg_id())==PdtPdgMini::Higgs0 && y1*y2>0) TauSpinEffectsH_phistarplus->Fill(phistar,weight);
   }
   /*   if(haspi_minus && haspi_plus){
     ...
     // fill histograms
     if(abs(boson->pdg_id())==PdtPdgMini::Higgs0) TauSpinEffectsH_phistarminus->Fill(phistar,weight);
     if(abs(boson->pdg_id())==PdtPdgMini::Higgs0) TauSpinEffectsH_phistarplus->Fill(phistar,weight);
     }*/
