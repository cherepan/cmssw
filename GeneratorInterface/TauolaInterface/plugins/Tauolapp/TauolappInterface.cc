...
void TauolappInterface::init( const edm::EventSetup& es ){
  ...
  fIsInitialized = true;
   ...
   //Filter information
   doFilter=fPSet->getUntrackedParameter<bool>("doFilter",false);
   doHelicityHadFilter=fPSet->getUntrackedParameter<bool>("doHelicityHadFilter",false);
   doHelicityLepFilter=fPSet->getUntrackedParameter<bool>("doHelicityLepFilter",false);
   hadHelicity=fPSet->getUntrackedParameter<int>("hadHelicity",-1);
   lepHelicity=fPSet->getUntrackedParameter<int>("lepHelicity",-1);
   doHadPtFilter=fPSet->getUntrackedParameter<bool>("doHadPtFilter",false);
   ...
   if(dolhe && lhe!=NULL){
	 ...
	 for(int i=0; i<ntries;i++){
	   ...
	   for(unsigned int j=0;j<spinup.size();j++){
	     if(abs(pdg.at(j))==15){
	       double diffhelminus=(-1.0*(double)Tauolapp::Tauola::getHelMinus()-spinup.at(j)); // -1.0 to correct for tauola feature 
	       double diffhelplus=((double)Tauolapp::Tauola::getHelPlus()-spinup.at(j));
	       if(pdg.at(j)==15  && diffhelminus>0.5) ismatch=false;
	       if(pdg.at(j)==-15 && diffhelplus>0.5)  ismatch=false;
	     }
	   }
	   delete t_event;
	   if(ismatch) break;
...
void TauolappInterface::BoostProdToLabLifeTimeInDecays(HepMC::GenParticle* p,TLorentzVector &lab, TLorentzVector &prod){
