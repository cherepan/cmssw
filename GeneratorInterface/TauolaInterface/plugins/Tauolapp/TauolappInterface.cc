...
void TauolappInterface::init( const edm::EventSetup& es ){
  ...
  Tauolapp::Tauola::setRandomGenerator(&TauolappInterface::flat);
  Tauolapp::Tauola::initialize();

  if(fPolarization==1) Tauolapp::Tauola::spin_correlation.setAll(fPolarization);// Tauola switches polarization off if requested

  // override decay modes if needs be
  // we have to do it AFTER init because otherwises branching ratios are NOT filled in
  if( fMDTAU !=0 && fMDTAU!=1) decodeMDTAU( fMDTAU );
...
double TauolappInterface::flat(){
  return (double)fRandomEngine->flat();
}
